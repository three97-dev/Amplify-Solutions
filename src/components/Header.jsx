...
const MobileHeaderSubMenuItem = ({ navLink }) => {
  const [isSubMenuOpen, setIsSubMenuOpen] = useState(false);

  return (
    <>
      {navLink?.subItems?.length ? (
        <div className="flex flex-col cursor-pointer" onClick={() => setIsSubMenuOpen(!isSubMenuOpen)}>
          <div className="flex items-center justify-end">
            ...
            {isSubMenuOpen && (
              <img src={ArrowHide} alt="ArrowHide" className="w-[14px] h-[7px] ml-[7px] relative z-[999]" />
            )}
          </div>
          {isSubMenuOpen && (
    ...
    <nav className="lg:absolute top-0 z-20 w-full">
      ...
      {isMenuOpen && (
        <div className="bg-dark-blue flex flex-col items-end pr-[24px] md:pr-[64px] gap-y-[20px] lg:hidden">
          {navLinks.map((navLink, index) => (
            <MobileHeaderSubMenuItem key={index} navLink={navLink} />
          ))}
        </div>
      )}
    </nav>
